...
public class SharedPrefsAutofillRepository implements AutofillDataSource {
    ...
    public HashMap<String, FilledAutofillFieldCollection> getFilledAutofillFieldCollection(
        ...
        Log.e("GIULIO","getFilledAutofillFieldCollection");

        boolean hasDataForFocusedAutofillHints = false;

        HashMap<String, FilledAutofillFieldCollection> clientFormDataMap = new HashMap<>();

        // IN QUESTO FOR HO TUTTI I FIELD PRECEDENTEMENTE INSERITI PER QUEL PACKAGE NAME
        for (Map.Entry<String, FilledAutofillFieldCollection> entry : clientFormDataMap.entrySet()) {
            String key = entry.getKey();
            FilledAutofillFieldCollection value = entry.getValue();
            Log.e("GIULIO clientFormDataMap chiave",key);
            Log.e("GIULIO clientFormDataMap valore",value.toString());


        }

        Set<String> clientFormDataStringSet = getAllAutofillDataStringSet(context);
        for (String s : clientFormDataStringSet) {
            Log.e("GIULIO clientFormDataStringSet valore",s.toString());
        }
    ...
    /*
    * Salvo qui i package che hanno precedentemente salvato le credenziali
    * */
